...
| `ablation_10_full_no_db_no_crop_dict_no_domain_filter` | off | off | on | on | on | off | on |
...
- `Inference/bash_generate.sh` now defaults to `ablation_10_full_no_db_no_crop_dict_no_domain_filter` and passes `--ablation_id` to the driver. For this condition it also explicitly disables the curated base collection and crop query enrichment, which are controlled by driver flags rather than the declarative `db_on` and `crop_dict_on` fields (`Inference/bash_generate.sh:37-58`, `Inference/bash_generate.sh:75-83`; `Inference/generate.py:860-930`).
- The repository verifies eight configuration records plus a separate `--no-rag` baseline path. It does not verify a complete nine-configuration experimental matrix or a launch harness that automatically iterates those conditions.
