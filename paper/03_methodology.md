...
MIRAGE denotes the underlying agricultural benchmark. We provisionally use *MIRAGE-CARMA* for the proposed system; its implementation lineage is called MIRAGE-CARMA or MIRAGE-CARMA in the repository artifacts, and the final manuscript name remains an author decision.
...
The confidence tool scores the structured retrieval result returned by the preceding retrieval call; it does not retrieve, embed, or call a model again. Retrieval uses $k=5$, a hard raw-cosine floor of $0.60$, and requires at least two chunks with raw cosine similarity at least $0.65$. For the semantically relevant chunks with similarities $s_1,\ldots,s_n$, it computes mean similarity $S$, relevant-evidence coverage $V$, relevance-scaled consistency $K$, and metadata scope $P$:
...
For one relevant passage, $K$ contributes zero. For multiple passages, consistency is scaled by semantic quality: $K=S\max(0,1-5\operatorname{Var}_{\mathrm{pop}}(s))$. The scope term assigns 1.0 to zone-month-title, 0.90 to zone-month, 0.85 to zone-title, 0.80 to zone, 0.75 to month, 0.70 to title, and 0.40 to semantic-only retrieval. The overall routing score is

\[
C=0.70S+0.15V+0.10K+0.05P.
\]

Scores are rounded to three decimal places and mapped to *high* when $C\geq0.78$, *medium* when $0.60\leq C<0.78$, and *low* otherwise. The hard-floor and minimum-relevant-result gates assign $C=0$ and *low* before weighted scoring when evidence is insufficient. Coverage measures relevant evidence quantity rather than semantic aspect coverage, and consistency measures dispersion in retrieval scores rather than factual agreement among sources. The weights and thresholds are initial design choices requiring empirical calibration. Because $S$ is an unclipped cosine score, $C$ is an uncalibrated routing heuristic rather than a probability or a measure of epistemic uncertainty.
...
The benchmark state and county are formatted as `[User location: <state>, <county>]` and prepended to the question. If query enrichment is enabled, its validated output becomes $q'_i$ for both evidence acquisition and final generation. The application records the structured retrieval evidence and uses its chunk text to construct final context; the terminal agent message is diagnostic only. Optional image combining places at most three images into labeled 512-by-512 panels and appends a panel-description hint.

The driver no longer infers retrieval validity from terminal response length. It records mutually exclusive structured statuses: `success`, `insufficient_evidence`, `invalid_output`, and explicit hard-failure categories for timeout, connection, model-service, and worker errors. Successful context is assembled from structured retrieved evidence; insufficient or invalid outcomes pass $q'_i$ to generation without appending a fabricated no-evidence sentence. Hard failures are retried by the pipeline layer and skipped after exhaustion.

## Behavioral Logging

The inference pipeline records an item-level summary of retrieval behavior in its JSONL output. For RAG-enabled runs, persisted fields identify the serving endpoint, retrieval attempt, structured RAG status, confidence, confidence score, web-search and ingestion activity, authoritative retrieval state, and diagnostic agent message. Direct-generation records instead mark RAG as `disabled` and unused. A successful response generation also stores the response model's message history. Function-call names remain transient execution traces.
